...
def open_and_read_pdf(pdf_path: str) -> List[Dict]:
    with open(pdf_path, 'rb') as file:
        ...
        return [
            ...
            for i, page in enumerate(tqdm.tqdm(pdf_reader.pages[8:], desc="Reading PDF (skipping first 8 pages)"), start=8)
        ]
...
def chunk_pdf_text(pages_and_texts: List[Dict], num_sentence_chunk_size: int = 4, overlap: int = 3) -> pd.DataFrame:
    all_chunks = []
...
def embed_chunks(df: pd.DataFrame, model: SentenceTransformer, min_token_length: int = 4) -> List[Dict]:
...
def embed_pdf_into_csv(pdf_path, embedding_model_name="sentence-transformers/all-MiniLM-L6-v2", csv_file_path="embeddings.csv"):
